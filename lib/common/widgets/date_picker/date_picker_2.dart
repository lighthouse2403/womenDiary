...
import 'package:women_diary/common/widgets/date_picker/calendar_date_picker2.dart';
...
class _DatePickerDialogState extends State<DatePickerDialog> with RestorationMixin {
  ...
  Widget build(BuildContext context) {
    ...
    CalendarDatePicker2 calendarDatePicker() {
...
///  * [DisplayFeatureSubScreen], which documents the specifics of how
///    [DisplayFeature]s can split the screen into sub-screens.
Future<DateTimeRange?> showDateRangePicker2({
  required BuildContext context,
...
/// (i.e. 'Jan 21, 2020').
String _formatRangeStartDate(MaterialLocalizations localizations, BuildContext context, DateTime? startDate) {
  return startDate == null
      ? localizations.dateRangeStartLabel
      : startDate.globalDateFormat();
}
...
String _formatRangeEndDate(MaterialLocalizations localizations, BuildContext context, DateTime? endDate) {
  return endDate == null
      ? localizations.dateRangeEndLabel
      : endDate.globalDateFormat();
}
...
class _CalendarRangePickerDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Color headerDisabledForeground = headerForeground.withAlpha(90);
    final String startDateText = _formatRangeStartDate(localizations, context, selectedStartDate);
    final String endDateText = _formatRangeEndDate(localizations, context, selectedEndDate);
      ...
      child: Scaffold(
        ...
        body: Column(
          children: [
            ...
            Container(
              padding: const EdgeInsets.only(top: 8, bottom: 12, left: 24, right: 16),
              child: Row(
                children: [
                  Expanded(
                    child: Row(
                      children: [
                        Flexible(child: Text(startDateText)),
                        const SizedBox(width: 16),
                        Assets.icons.arrowDown.svg(colorFilter: ColorFilter.mode(Constants.primaryTextColor(), BlendMode.srcIn)),
                        const SizedBox(width: 16),
                        Flexible(child: Text(endDateText)),
                      ],
                    ),
                  ),
                  const SizedBox(width: 8),
                  TextButton(onPressed: onConfirm, child: Text(localizations.okButtonLabel))
                ],
              ),
            )
          ],
...
class _MonthItemState extends State<_MonthItem> {
  ...
  Widget _buildDayItem(BuildContext context, DateTime dayToBuild, int firstDayOffset, int daysInMonth) {
    ...
    if (isSelectedDayStart || isSelectedDayEnd) {
      ...
      itemStyle = textTheme.bodyMedium?.apply(color: colorScheme.onSurface.withAlpha(90));
    ...
    if (!isDisabled) {
      dayWidget = InkResponse(
        ...
        splashColor: colorScheme.primary.withAlpha(90),
...
class _InputDateRangePickerDialog extends StatelessWidget {
  ...
  String _formatDateRange(BuildContext context, DateTime? start, DateTime? end, DateTime now) {
    final MaterialLocalizations localizations = MaterialLocalizations.of(context);
    final String startText = _formatRangeStartDate(localizations, context, start);
    final String endText = _formatRangeEndDate(localizations, context, end);
